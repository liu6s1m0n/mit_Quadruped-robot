...
private:
  ...
  template < typename T >
  inline void GMatr < T > ::setColumns(
  ...
  template < typename T >
  inline void GMatr < T > ::set(
    const std::set < unsigned int > &r_indexes,
    const std::set < unsigned int > &c_indexes, const GMatr < T > &a)
  {
    unsigned int i = 0, j;
  ...
  */
  ...
  GMatr < T > pinv(const GMatr < T > &A)
  {
    ...
    for (unsigned int i = 0; i < A.nrows(); i++) {
      ...
      tmp.setColumn(
        ...
        dot_prod(
          dot_prod(
            dot_prod(V, GMatr < double > (DIAG, 1.0 / W, 0.0, W.size(), W.size())),
            t(U)), e));
    }
  ...
  template < typename T >
  GVect < T > lu_solve_gold(
    const GMatr < T > &LU, const GVect < T > &b,
  ...
  template < typename T >
  void lu_solve_gold(
    const GMatr < T > & LU, GVect < T > & x, const GVect < T > & b,
    GVect < unsigned int > & index)
  {
