...
bool add_constraint(
  GMatr < double > & R, GMatr < double > & J, GVect < double > & d, int & iq,
  double & rnorm);
void delete_constraint(
  GMatr < double > & R, GMatr < double > & J, GVect < int > & A,
...
void forward_elimination(
  const GMatr < double > & L, GVect < double > & y,
  const GVect < double > & b);
void backward_elimination(
  const GMatr < double > & U, GVect < double > & x,
...
double solve_quadprog(
  Eigen::MatrixXd & _G, Eigen::VectorXd & _g0,
...
double solve_quadprog(
  GMatr < double > & G, GVect < double > & g0,
  ...
  GVect < double > & x)
{
  ...
  if (G.nrows() != static_cast < unsigned int > (n)) {
    msg << "The matrix G is not a squared matrix (" << G.nrows() << " x " << G.ncols() << ")";
    throw std::logic_error(msg.str());
  }
  if (CE.nrows() != static_cast < unsigned int > (n)) {
    msg << "The matrix CE is incompatible (incorrect number of rows " << CE.nrows() <<
      " , expecting " << n << ")";
    throw std::logic_error(msg.str());
  }
  if (ce0.size() != static_cast < unsigned int > (p)) {
    msg << "The vector ce0 is incompatible (incorrect dimension " << ce0.size() << ", expecting " <<
      p << ")";
    throw std::logic_error(msg.str());
  }
  if (CI.nrows() != static_cast < unsigned int > (n)) {
    msg << "The matrix CI is incompatible (incorrect number of rows " << CI.nrows() <<
      " , expecting " << n << ")";
    throw std::logic_error(msg.str());
  }
  if (ci0.size() != static_cast < unsigned int > (m)) {
    msg << "The vector ci0 is incompatible (incorrect dimension " << ci0.size() << ", expecting " <<
      m << ")";
    throw std::logic_error(msg.str());
  }
  x.resize(n);
...
inline void update_z(
  GVect < double > & z, const GMatr < double > & J, const GVect < double > & d,
  int iq)
{
...
inline void update_r(
  const GMatr < double > & R, GVect < double > & r, const GVect < double > & d,
  int iq)
{
  register int i, j;
  register double sum;
